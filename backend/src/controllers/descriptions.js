...
function suggest(req, res) {
  ...
  if (category_id && (!q || q.length < 2)) {
    ...
    results = db.prepare(`
      SELECT description, usage_count, 0 as category_usage, NULL as vat_rate
...
function track(req, res) {
  const db = getDatabase();
  const { description, category_id, vat_rate } = req.body;
  ...
  // Upsert description_category_usage with vat_rate
  if (category_id) {
    db.prepare(`
      INSERT INTO description_category_usage (description, category_id, usage_count, last_used_at, vat_rate)
      VALUES (?, ?, 1, ?, ?)
      ON CONFLICT(description, category_id) DO UPDATE SET
        usage_count = usage_count + 1,
        last_used_at = ?,
        vat_rate = ?
    `).run(description, category_id, now, vat_rate ?? null, now, vat_rate ?? null);
  }
