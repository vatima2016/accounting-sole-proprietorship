...
import { parseAmountExpression } from '../../utils/validation';

export default function CalculationDisplay({ grossAmount, vatRate }) {
  const { total, parts, isNet } = parseAmountExpression(grossAmount);
  ...
  return (
    <div className="bg-blue-50 rounded-lg p-3 text-sm space-y-1">
      {hasMultiple && (
        <div className="flex justify-between text-gray-500">
          <span>{parts.map(p => formatCurrency(p)).join(' + ')}</span>
          <span>=</span>
        </div>
      )}
      <div className="flex justify-between">
